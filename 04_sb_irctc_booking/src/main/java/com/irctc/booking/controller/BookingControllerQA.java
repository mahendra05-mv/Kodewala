package com.irctc.booking.controller;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.annotation.Profile;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.irctc.booking.entity.BookingEntity;
import com.irctc.booking.response.BookingResponse;
import com.irctc.booking.service.BookingServiceQA;

//first git commit

@RestController
@RequestMapping("irctc")
@Profile("QA")
public class BookingControllerQA {

	@Autowired
	BookingServiceQA serviceQA;

	@PostMapping("/changeStatus/{pnr}")
	public BookingEntity changeStatus(@PathVariable long pnr) {
		
		return serviceQA.confirmStatus(pnr);
	}

	@PostMapping("/displayDetails/{pnr}")
	public BookingResponse displayDetails(@PathVariable long pnr) {
		return serviceQA.displayDetails(pnr);

	}
}
